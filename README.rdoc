...
* ログインした状態で http://localhost:3000 にアクセスし、定期課金を作成する事が出来ます。

* 作成した定期課金は http://localhost:3000/recursions で確認できます。

* https://webpay.jp/settings 下部より、フック先のURLを http://localhost:3000/webhook に設定したWebhookを設定しておくと、定期課金によって行われた課金が発生した時にそのitemの買い物履歴を作成します。
